...
path='C:\Users\zhxing\Desktop\cv_tracking\附件3-检测前景视频\overpass\overpass.avi';
...
for i=1:frame_n                              %直接用每一帧和背景做差。
    ...
    subplot(2,2,4);
    plot(hist_cha);
    title('当前帧与背景直方图相似度');             %这里是绝对值。不是相对值
end

all_time=toc;
disp('处理的帧率为：');
floor(frame_n/floor(toc)),disp('fps')
